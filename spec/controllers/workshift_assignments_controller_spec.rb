...
describe WorkshiftAssignmentsController do
  ...
  it 'doesnt check off if the assignment cant be checked off' do
    @workshift_assignment = create(:workshift_assignment, id: 1, workshifter: @user)
  ...
  it 'doesnt check off if the verifier is the same user' do
    @workshift_assignment = create(:workshift_assignment, id: 1, workshifter: @user, status: "awaiting check off")
  ...
  it 'checks off correctly' do
    ...
    @workshift_assignment = create(:workshift_assignment, id: 1, workshifter: @user, status: "awaiting check off", verifier: another_user)
    WorkshiftAssignment.any_instance.should_receive(:check_off)
    ...
    post :check_off, id: 1, verifier: 2
    expect(response).to redirect_to user_profile_path(@workshift_assignment.workshifter)
  end

  it 'successfully puts an assignment on the market' do
    @workshift_assignment = create(:workshift_assignment, id: 1, workshifter: @user, status: "upcoming")
  ...
  it 'doesnt sell one if current user is unauthorized' do
    another_user = create(:user, id: 2, email: 'lol@lol.com')
    @workshift_assignment = create(:workshift_assignment, id: 1, workshifter: another_user, status: "awaiting check off")
